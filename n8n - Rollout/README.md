...
| Item | Purpose |
|------|--------|
| `workflows/rollout-drive-watch.json` | Runs on a schedule; lists files in your policy folder that are new or modified since the last run; triggers the ingest workflow for each file. |
| `workflows/rollout-ingest.json` | Triggered by the watch workflow (or manually for backfill): downloads each file, extracts text, chunks, embeds, and upserts into the vector store. |
| `workflows/rollout-policy-ask-workflow.json` | Webhook for employee questions: retrieves relevant chunks from the vector store and returns a grounded answer (with optional Structured Governor). |
| `docs/` | Internal notes (chunking, folder ID, etc.). |
...
2. Import in this order (so IDs and names match):
   - `workflows/rollout-ingest.json`
   - `workflows/rollout-drive-watch.json`
   - `workflows/rollout-policy-ask-workflow.json`
